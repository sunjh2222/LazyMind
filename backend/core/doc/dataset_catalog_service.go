...
func (s *DatasetCatalogService) ListDatasets(ctx context.Context, req DatasetListRequest) (DatasetListResult, error) {
	...
	if sourceFilter != datasetSourceCloud && sourceFilter != datasetSourceManual && sourceFilter != datasetSourceOfficialInstalled {
		sourceFilter = ""
	}
	caller := req.Caller
	...
	for hasMoreRows {
		...
		if len(candidates) > 0 {
			...
			sourceTypeMap := buildDatasetSourceTypeMap(
				candidateIDs,
				batchCheckDatasetsHaveSource(ctx, candidateIDs),
				batchCheckInstalledMarketDatasets(ctx, userID, candidateIDs),
			)
	...
	out := s.datasetsFromRows(ctx, page, userID, groupIDs, pageSourceTypeMap)
	end := offset + len(page)
...
func (s *DatasetCatalogService) GetDataset(ctx context.Context, req DatasetGetRequest) (Dataset, error) {
	...
	sourceType := datasetSourceManual
	if createdByDataSource {
		sourceType = datasetSourceCloud
	}
	if batchCheckInstalledMarketDatasets(ctx, userID, []string{ds.ID})[ds.ID] {
		sourceType = datasetSourceOfficialInstalled
	}
	return s.datasetFromRow(ctx, ds, userID, datasetACL, calcDatasetStatsWithDB(ctx, s.db, ds.ID), createdByDataSource, sourceType, nil), nil
}

func (s *DatasetCatalogService) datasetsFromRows(ctx context.Context, rows []orm.Dataset, userID string, groupIDs []string, sourceTypeMap map[string]string) []Dataset {
	...
	for _, ds := range rows {
		...
		sourceType := sourceTypeMap[ds.ID]
		out = append(out, s.datasetFromRow(ctx, ds, userID, datasetACL, statsMap[ds.ID], sourceType == datasetSourceCloud, sourceType, liveParsers))
	}
	return out
}
